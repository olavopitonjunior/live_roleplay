...
// Re-export useEmotion for backward compatibility
export { useEmotion } from '../../hooks/useTranscript';
...
const HIGHLIGHT_KEYWORDS = [
  // Objections
  'caro', 'preco', 'orcamento', 'pensar', 'depois', 'nao sei', 'duvida',
  // Positive signals
...
/**
 * TranscriptionOverlay displays the conversation messages.
 * Uses useTranscript hook to get messages from shared context.
 * This ensures messages persist when switching tabs.
 */
export function TranscriptionOverlay({ variant = 'overlay' }: TranscriptionOverlayProps) {
  const { messages, agentStatus } = useTranscript();
      ...
      <div ref={scrollRef} className="max-h-40 overflow-y-auto space-y-2 scrollbar-hide">
        ...
        {messages.length === 0 && (
          <div className="text-center text-gray-400 text-sm py-4 bg-gray-900/60 rounded-lg">
            A transcricao aparecera aqui durante a conversa
          </div>
        )}
      </div>
    </div>
